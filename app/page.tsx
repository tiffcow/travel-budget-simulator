...
import './globals.css';
import { defaultMultipliers, fetchTeleportMultiplier, type CountryKey } from '../lib/costMultipliers';
...
export default function Page() {
  ...
  const [countries, setCountries] = useState<CountryConfig[]>(
    START_COUNTRIES.map((c) => ({ country: c, months: 1, multiplier: defaultMultipliers[c] }))
  );
  const [inputs, setInputs] = useState<Inputs>({
    baselinePerPersonUSD: 2100,
    extrasPerPersonUSD: 700,
    numPeople: 1,
    sharedBaselineFraction: 0.6,
    sharedExtrasFraction: 0.1,
    monthlyIncomeUSD: 0,
    emergencyPercent: 8
  });
  ...
  async function refreshTeleport() {
    setLoadingTeleport(true);
    try {
      const updated = await Promise.all(
        countries.map(async (c) => {
          const key = c.country as CountryKey;
          const m = await fetchTeleportMultiplier(key);
          return { ...c, multiplier: m ?? c.multiplier };
        })
      );
      setCountries(updated);
    } finally {
      setLoadingTeleport(false);
    }
  }

  const outputs = useMemo(() => computePlan(countries, inputs), [countries, inputs]);

  function setCountryMonths(i: number, months: number) {
    const list = [...countries];
    list[i] = { ...list[i], months: Math.max(0, Math.floor(months || 0)) };
    setCountries(list);
  }
  function setCountryMultiplier(i: number, val: number) {
    const list = [...countries];
    list[i] = { ...list[i], multiplier: Math.max(0.1, Number(val) || 0.1) };
    setCountries(list);
  }
    ...
    <main className="min-h-screen p-6 md:p-10 bg-[var(--bg)]">
      <div className="max-w-6xl mx-auto grid gap-6">
        <header className="flex items-center justify-between">
          ...
          <button
            onClick={refreshTeleport}
            disabled={loadingTeleport}
            className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/15 transition text-sm"
          >
            {loadingTeleport ? 'Refreshing…' : 'Use live multipliers'}
          </button>
        </header>
        ...
        <section className="grid md:grid-cols-3 gap-4">
          <div className="bg-[#111827] rounded-2xl p-4 space-y-3">
            ...
            <input
              ...
              onChange={(e) =>
                setInputs((s) => ({ ...s, numPeople: Math.max(1, Math.min(2, Number(e.target.value) || 1)) }))
              }
            ...
            <input
              type="number"
              value={inputs.baselinePerPersonUSD}
              onChange={(e) => setInputs((s) => ({ ...s, baselinePerPersonUSD: Number(e.target.value) || 0 }))}
              className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
            />
            <label className="block text-sm text-[#9ca3af]">Extras per person USD</label>
            <input
              type="number"
              value={inputs.extrasPerPersonUSD}
              onChange={(e) => setInputs((s) => ({ ...s, extrasPerPersonUSD: Number(e.target.value) || 0 }))}
              className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
            />
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  min={0}
                  max={100}
                  value={Math.round(inputs.sharedBaselineFraction * 100)}
                  onChange={(e) =>
                    setInputs((s) => ({
                      ...s,
                      sharedBaselineFraction: Math.max(0, Math.min(100, Number(e.target.value) || 0)) / 100
                    }))
                  }
                  className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
                />
              ...
              <div>
                ...
                <input
                  ...
                  min={0}
                  max={100}
                  value={Math.round(inputs.sharedExtrasFraction * 100)}
                  onChange={(e) =>
                    setInputs((s) => ({
                      ...s,
                      sharedExtrasFraction: Math.max(0, Math.min(100, Number(e.target.value) || 0)) / 100
                    }))
                  }
                  className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
                />
          ...
          <div className="bg-[#111827] rounded-2xl p-4 space-y-3">
            ...
            <input
              type="number"
              value={inputs.monthlyIncomeUSD}
              onChange={(e) => setInputs((s) => ({ ...s, monthlyIncomeUSD: Number(e.target.value) || 0 }))}
              className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
            />
            ...
            <input
              ...
              onChange={(e) =>
                setInputs((s) => ({ ...s, emergencyPercent: Math.max(0, Math.min(100, Number(e.target.value) || 0)) }))
              }
              className="w-full bg-transparent border border-white/10 rounded-xl px-3 py-2"
            />
          ...
          <div className="bg-[#111827] rounded-2xl p-4 space-y-3">
            ...
            <div className="space-y-1 text-sm">
              ...
              {inputs.numPeople === 2 && (
                <div>Partner share: <span className="text-white">${outputs.totals.partnerShareUSD.toFixed(0)}</span></div>
              )}
            </div>
          </div>
        </section>
            ...
            {countries.map((c, idx) => (
              <div key={idx} className="grid grid-cols-12 items-center gap-3 bg-white/5 rounded-xl p-3">
                ...
                <div className="col-span-12 md:col-span-3 text-sm text-right">
                  <div>
                    Baseline: <span className="text-white">
                      {(inputs.baselinePerPersonUSD * c.multiplier).toFixed(0)}/mo
                    </span>
                  </div>
                  <div>
                    Extras: <span className="text-white">
                      {(inputs.extrasPerPersonUSD * c.multiplier).toFixed(0)}/mo
                    </span>
                  </div>
                </div>
              </div>
